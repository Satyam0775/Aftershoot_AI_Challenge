...
I implemented and trained the full pipeline in two different environments — VS Code (local CPU) and Google Colab (GPU) — to analyze performance and convergence differences.
...
Image Branch: Used a pretrained ResNet18 (from torchvision.models) for extracting visual embeddings from each 256×256 TIFF image.
Metadata Branch: Built a lightweight Multi-Layer Perceptron (MLP) to process numerical EXIF parameters such as
...
Optimizer: Adam optimizer with learning rate = 1e-4 and a StepLR scheduler (step_size=5, gamma=0.5).
...
EnvironmentDeviceTraining Time (20 Epochs)Batch SizeNotesVS Code (Local)CPU (Intel Core i5)⏱ ~113 minutes16Training slower due to CPU-only computation, but stable convergence.Google Colab (Cloud)GPU (Tesla T4, CUDA)⚡ ~20 minutes32Much faster due to GPU acceleration, identical loss curve and results.
...
Temperature Range: 2000–12000K
Tint Range: -20 to +30
Predictions closely aligned with human-edited color tone distributions.

📊 Results Summary
VersionEnvironmentFinal MAETraining DurationFileVS Code (CPU)Local0.15113 minsubmissions/predictions.csvGoogle Colab (GPU)Cloud0.1320 minsubmissions/Colab_predictions.csv
